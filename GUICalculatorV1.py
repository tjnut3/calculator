...
# โครตสำคัญ !!!!!!!!!!!
# ตัวค่าที่ต้องส่งออกไปใช้งานคือตัวแปร value บรรทัดที่ 34 และรับค่าที่คำนวณเสร็จ 37
...
def save_value():
    ...
    getValue.insert(0,"ผลลัพธ์ที่ได้") # ผลลัพธ์ที่ได้มาใส่ตรงนี้

# ช่องในการรับค่า
textValue = StringVar()
getValue = Entry(screen_frame,textvariable=textValue,width=20,justify="right",font=entry_font)
getValue.pack(fill=BOTH, expand=True, padx=10, pady=10)
...
# แถวที่ 1 ปุ่ม c , <-
clear = Button(button_frame,text="C",font=button_font,width=4, command=lambda: getValue.delete(0,END)).grid(row=1, column=2, sticky="nsew")
